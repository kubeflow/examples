...
```
kustomize edit add configmap mnist-map-local --from-literal=trainSteps=200
kustomize edit add configmap mnist-map-local --from-literal=batchSize=100
kustomize edit add configmap mnist-map-local --from-literal=learningRate=0.01
```
...
kustomize edit add configmap mnist-map-local --from-literal=pvcMountPath=/mnt
```

Now we need to configure parameters and telling the code to save the model to PVC.

```
kustomize edit add configmap mnist-map-local --from-literal=modelDir=/mnt
kustomize edit add configmap mnist-map-local --from-literal=exportDir=/mnt/export
```
...
```
kustomize edit add configmap mnist-map-gcs --from-literal=trainSteps=200
kustomize edit add configmap mnist-map-gcs --from-literal=batchSize=100
kustomize edit add configmap mnist-map-gcs --from-literal=learningRate=0.01
```
...
kustomize edit add configmap mnist-map-serving-local --from-literal=pvcMountPath=/mnt
```

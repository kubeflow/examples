  ...
  template:
    ...
    spec:
      ...
      - command:
        ...
        - --logdir=gs://jlewi-dev-mnist/mnist/export
        - --port=80
...
spec:
  gateways:
  - kubeflow/kubeflow-gat
  hosts:
  - '*'
  http:
  - match:
    - uri:
        prefix: /mnist/kubeflow-jlewi/tensorboard/
    rewrite:
      uri: /mnist/kubeflow-jlewi/tensorboard/
    route:
    - destination:
        host: mnist-tensorboard.kubeflow-jlewi.svc.cluster.local
        port:
          number: 80
    timeout: 300s
"""]

  apply_k8s_specs(spec, mode=K8S_CREATE_OR_REPLACE)

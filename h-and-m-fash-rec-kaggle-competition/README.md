...
5. Now before heading to KFP steps, we need to save our Kaggle API credentials as a secret so that we can use the Kaggle Public [API](https://github.com/Kaggle/kaggle-api/blob/master/kaggle/api/kaggle_api_extended.py) to download the files from the Kaggle competition for our KFP/Kale pipeline. Following are the steps:
    ...
    - Once you have the API credentials, run the following command in the terminal with the username and key from kaggle.json file that you just saved

    ```
        kubectl create secret generic kaggle-secret --from-literal=KAGGLE_USERNAME=<username> --from-literal=KAGGLE_KEY=<api_token> 

    ``` 
    This creates a secret for our credentials which can then be mounted on out pods.
...
6. With the completion of 5th step, you are good to start with KFP steps.

...
def create_job(args, app_dir): #pylint:disable=redefined-outer-name
  ...
  util.run(['wget', '-O', '/usr/local/bin/kustomize', kusUrl], cwd=app_dir)
  ...
  util.run(['gcloud', '--project=kubeflow-ci-deployment', 'container',
          "clusters", "get-credentials", "--zone=us-east1-b", "kf-vmaster-n00"], cwd=app_dir)
  ...
  util.run(['kustomize', 'edit', 'add', 'configmap', configmap,
          '--from-literal=image=' + args.image], cwd=app_dir)
  #util.run does not support pipe.
  subCmd = 'kustomize build ' + app_dir + '| kubectl apply -f -'

...
def prepare_env():
  subprocess.check_call(["git", "clone", "https://github.com/kubeflow/examples.git"])
  os.chdir("./examples")
  subprocess.check_call(["git", "fetch", "origin",
                        "pull/" + os.getenv("PULL_NUMBER") + "/head:pr"])
  subprocess.check_call(["git", "checkout", "pr"])
  subprocess.check_call(["pip3", "install", "-U", "papermill"])
  subprocess.check_call(["pip3", "install", "-r",
                        "./xgboost_synthetic/requirements.txt"])
...
if __name__ == "__main__":
  ...
  NOTEBOOK_REL_PATH = "./xgboost_synthetic/build-train-deploy.ipynb"
  NOTEBOOK_ABS_PATH = os.path.normpath(os.getcwd() + '/' + NOTEBOOK_REL_PATH)
  EXPECTED_MGS = [
      "Finished upload of",
